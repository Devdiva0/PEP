...
create table customer( customer_id int primary key auto_increment, first_name varchar(50) not null, last_name varchar(50) not null, email varchar(100) unique, city varchar(50), state varchar(50), signup_date date default(current_date) );
...
create table departments( department_id int primary key auto_increment, department_name varchar(50) not null, description varchar(100) not null);
...
create table order_items( order_item_id int primary key auto_increment, order_id int, product_id int, quantity int check(quantity>0), unit_price decimal(10,2) not null, foreign key (order_id) references orders(order_id), foreign key (product_id) references products(product_id) );
...
CREATE TABLE orders (     order_id INT PRIMARY KEY AUTO_INCREMENT,     customer_id INT,     order_date DATE DEFAULT (CURRENT_DATE), status varchar(20) default 'PENDING',     FOREIGN KEY (customer_id) REFERENCES customer(customer_id) );
...
oi.quantity*oi.unit_price as 'line total' 
from customer c 
left join orders o 
